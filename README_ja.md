...
|---|---:|---:|
| 中央値 | 48 µs | 53 µs |
| 90 パーセンタイル | 67 µs | 75 µs |
| 丸ごとのパース（目安） | 1.7 ms | |

中央値でも裾でも上回っています。裾は match の腕と impl のメンバを item にする前は 383 µs、
doc コメント内の編集（トークンに触れない）が直前の item を読み直さなくなる前は 219 µs、名前への
1 文字の打ち込み（1 トークンの打ち直し）がその item を読み直さなくなる前は 137 µs でした。コンパイラの追跡下 `.rs` 1,015 ファイルに各 10 回のランダム編集
