...
|---|---:|---:|
| 残った宣言(全破壊) | 99.9% | 97.5% |
| きれいに回復した破壊(壊した箇所以外を失わず、余計なものも出さない) | 99.8% | 94.9% |
| きれいに回復、`insert {` | 99.8% | 97.1% |
| きれいに回復、`delete }` | 99.4% | 94.5% |
| きれいに回復、`delete )` | 100.0% | 92.7% |
| きれいに回復、`insert (` | 100.0% | 95.2% |

4 通りすべてで gramide が上。`}` を消すと次の `fn` や item で再開し、tree-sitter は後続を開いた
本体に入れ子にする。`)` を消しても gramide が失うのはそれを含む item だけ。
