...
|---|---:|---:|
| 中央値 | 56 µs | 54 µs |
| 90 パーセンタイル | 137 µs | 78 µs |
| 丸ごとのパース（目安） | 1.7 ms | |

中央値は同等、裾では負けています。裾の編集は大きな関数の名前に落ちたもので、item 自身のトークンが
変わるのでその item を丸ごと読み直します。裾は match の腕と impl のメンバを item にする前は 383 µs、
doc コメント内の編集（トークンに触れない）が直前の item を読み直さなくなる前は 219 µs でした。コンパイラの追跡下 `.rs` 1,015 ファイルに各 10 回のランダム編集
（10,000 回、毎回トークンとノードを丸ごとのパースと照合）で差はゼロ、13 回はファイル全体を読みました
