...
|---|---:|---:|
| 中央値 | 4.6 µs | 53 µs |
| 90 パーセンタイル | 6.2 µs | 75 µs |
| 丸ごとのパース（目安） | 1.6 ms | |

中央値で 11 倍速い。裾は match の腕と impl のメンバを item にする前は 383 µs、
...
なかった破壊を「きれい」と数える([証拠](docs/evidence/recovery-almide-compiler-rs.json)、[仕組み](https://github.com/O6lvl4/gramide/blob/main/docs/recovery.md)):

| Almide compiler `crates/`: 663 ファイル、2,632 回の破壊 | gramide | tree-sitter |
|---|---:|---:|
| 残った宣言(全破壊) | 100.0% | 97.5% |
| きれいに回復した破壊(壊した箇所以外を失わず、余計なものも出さない) | 100.0% | 94.9% |
| きれいに回復、`insert {` | 100.0% | 97.1% |
| きれいに回復、`delete }` | 100.0% | 94.5% |
| きれいに回復、`delete )` | 100.0% | 92.7% |
| きれいに回復、`insert (` | 99.8% | 95.2% |

4 通りすべてで gramide が上で、2,632 回の破壊で壊した箇所の外で失った宣言は 2 つ。`}` を消しても
ブロックはインデントの示す位置で終わり、tree-sitter は後続を開いた本体に入れ子にする。`)` を
消しても、それが属する呼び出しやシグネチャの終わりで閉じる。
